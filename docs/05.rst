...
uWSGI + nginx
=============
* uwsgi（とgunicorn）はわりとよく使われていそうなuWSGIサーバー。
* nginxからstaticファイルを直接配信させたり、キャッシュを使ったりできる。
* ただし、Linux, nginxに関する知識も必要となる。
* uwsgiサーバーを自動起動・自動再起動するのに他の仕組みを使う必要がある。


uwsgiのインストール

`Quickstart for python/WSGI applications — uWSGI 1.9 documentation <http://uwsgi-docs.readthedocs.org/en/latest/WSGIquickstart.html>`_
...
/path/to/flaskrにflaskrディレクトリを置く::
...
* PaaSを使えば、サーバーとかそういうのを意識する必要がない！
* herokuの枠を外れることをしようとすると諦める or がんばる必要がある。
